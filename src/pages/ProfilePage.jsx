/**
 * ProfilePage — v5 Polish
 * Clean profile. FCS-forward language.
 */
...
export default function ProfilePage() {
  ...
  async function handleSave() {
    await updateProfile({ display_name: displayName || null, timezone: timezone, currency: currency, risk_style: riskStyle })
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: FONT, display: "flex", flexDirection: "column", alignItems: "center", padding: "40px 24px" }}>
      ...
      <div style={{ width: "100%", maxWidth: 560 }}>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <p style={{ fontSize: 13, color: C.dim, margin: 0 }}>This is how Grace knows you and personalizes your experience.</p>
        </div>
        ...
        <div style={cardStyle}>
          <div style={sectionLabel}>Financial Profile</div>
          <label style={{ ...labelStyle, marginBottom: 12 }}>Risk Tolerance</label>
        ...
        <div style={cardStyle}>
          ...
          <p style={{ fontSize: 13, color: C.dim, lineHeight: 1.7, margin: "0 0 16px" }}>Your data is encrypted and never sold. You own it completely.</p>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {["Export my data (CSV)", "Delete my account"].map(function (action, i) {
              var isDanger = i === 1
              return (<button key={i} style={{ padding: "12px 16px", borderRadius: 8, fontSize: 13, fontWeight: 500, fontFamily: FONT, cursor: "pointer", textAlign: "left", background: "transparent", border: "1px solid " + (isDanger ? "#331111" : C.border), color: isDanger ? "#ff4444" : C.dim, transition: "all 0.2s" }}>{action}</button>)
            })}
          </div>
        </div>
