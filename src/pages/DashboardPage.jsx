...
function apiFetch(endpoint, options) {
  ...
  if (options) { for (var k in options) { if (k === "headers") { for (var h in options.headers) headers[h] = options.headers[h] } else { config[k] = options[k] } } }
...
function Card(props) { return (<div style={{ background: C.card, border: "1px solid " + C.border, borderRadius: 12, padding: props.mobilePad || 24, ...(props.style || {}) }}>{props.children}</div>) }
function Label(props) { return (<span style={{ fontSize: 11, fontWeight: 500, color: C.muted, textTransform: "uppercase", letterSpacing: "0.08em", fontFamily: FONT }}>{props.children}</span>) }
...
function Nav(props) {
  ...
  return (
    <div style={{ display: "flex", gap: 0, borderBottom: "1px solid " + C.border, marginBottom: props.isMobile ? 20 : 32, overflowX: "auto", WebkitOverflowScrolling: "touch", msOverflowStyle: "none", scrollbarWidth: "none" }}>
      <style>{".nav-scroll::-webkit-scrollbar { display: none; }"}</style>
      {items.map(function (item) {
        var isActive = props.active === item.id
        return (<button key={item.id} onClick={function () { props.navigate(item.path) }} style={{
          padding: props.isMobile ? "10px 14px" : "12px 20px", fontSize: 13, fontWeight: isActive ? 600 : 400,
          fontFamily: FONT, color: isActive ? C.text : C.dim, background: "transparent", border: "none",
          borderBottom: isActive ? "1px solid " + C.text : "1px solid transparent", cursor: "pointer",
          transition: "color 0.2s ease", letterSpacing: "0.01em", marginBottom: -1, whiteSpace: "nowrap", flexShrink: 0,
        }}>{item.label}</button>)
      })}
    </div>
  )
...
function FCSScore(props) {
  ...
  return (
    <Card style={{ textAlign: "center", padding: isMobile ? "28px 20px" : "36px 24px" }}>
      <Label>Financial Confidence Score</Label>
      <div style={{ position: "relative", width: size, height: size, margin: "20px auto 16px" }}>
        <svg width={size} height={size} style={{ position: "absolute", top: 0, left: 0 }}>
          <circle cx={size/2} cy={size/2} r={radius} fill="none" stroke={C.border} strokeWidth="4" />
          <circle cx={size/2} cy={size/2} r={radius} fill="none" stroke={C.text} strokeWidth="4" strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={offset} style={{ transition: "stroke-dashoffset 1.2s cubic-bezier(0.4, 0, 0.2, 1)", transform: "rotate(-90deg)", transformOrigin: "50% 50%" }} />
        </svg>
      ...
      {trend !== null && trend !== undefined && (
        <div style={{ fontSize: 13, color: C.muted, fontFamily: FONT, marginBottom: 4 }}>
          <span style={{ color: C.text, fontWeight: 600, fontVariantNumeric: "tabular-nums" }}>{trend >= 0 ? "+" : ""}{typeof trend === "number" ? trend.toFixed(1) : trend}</span>
          <span style={{ marginLeft: 6 }}>since last check-in</span>
        </div>
      )}
      <p style={{ fontSize: 13, color: C.dim, margin: "12px 0 0", lineHeight: 1.7, fontFamily: FONT }}>{score != null ? getScoreMessage(score) : "Your Financial Confidence Score will appear after your first check-in."}</p>
      {checkinCount > 0 && (<p style={{ fontSize: 11, color: C.faint, marginTop: 8, fontFamily: FONT }}>Calculated from {checkinCount} responses this week</p>)}
    </Card>
  )
}
    ...
    <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr 1fr" : "1fr 1fr 1fr", gap: isMobile ? 8 : 12 }}>
      {stats.map(function (s) {
        return (<Card key={s.label} style={{ padding: isMobile ? "14px 12px" : "20px" }}>
          <Label>{s.label}</Label>
          <div style={{ fontSize: isMobile ? 22 : 28, fontWeight: 600, color: C.text, letterSpacing: "-0.03em", marginTop: 6, fontFamily: FONT, fontVariantNumeric: "tabular-nums" }}>{s.value}</div>
          <div style={{ fontSize: isMobile ? 10 : 12, color: C.dim, marginTop: 3, fontFamily: FONT }}>{s.sub}</div>
        </Card>)
      })}
    </div>
  )
    ...
    <Card>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        {dims.map(function (dim) {
          var val = metrics[dim]; var pct = val != null ? Math.round(val * 100) : null; var weight = Math.round(dimensionWeights[dim] * 100)
          return (<div key={dim}>
  ...
  return (
    <Card>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        ...
        <div style={{ textAlign: "right" }}><div style={{ fontSize: 20, fontWeight: 600, color: C.text, fontFamily: FONT, fontVariantNumeric: "tabular-nums", letterSpacing: "-0.02em" }}>{avg}</div><div style={{ fontSize: 10, color: C.dim, textTransform: "uppercase", letterSpacing: "0.06em" }}>30-Day Avg</div></div>
      </div>
  ...
  return (
    <Card>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 5, height: 5, borderRadius: "50%", background: C.text }} /><span style={{ fontSize: 10, color: C.muted, textTransform: "uppercase", letterSpacing: "0.06em" }}>Ready</span></div>
      </div>
...
export default function DashboardPage() {
  ...
  var dateStr = screen.isMobile
    ? todayDate.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })
    : todayDate.toLocaleDateString("en-US", { weekday: "long", year: "numeric", month: "long", day: "numeric" })

  var pad = screen.isMobile ? "16px 16px 60px" : "24px 24px 60px"
  var gap = screen.isMobile ? 16 : 28
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: pad }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, opacity: mounted ? 1 : 0, transition: "opacity 0.5s ease" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            {!screen.isMobile && <span style={{ fontSize: 14, fontWeight: 600, letterSpacing: "-0.02em" }}>GraceFinance</span>}
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: screen.isMobile ? 10 : 16 }}>
            {!screen.isMobile && <span style={{ fontSize: 12, color: C.dim }}>{user && user.first_name ? user.first_name : ""}</span>}
