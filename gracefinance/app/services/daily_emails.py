...
═══════════════════════════════════════
v1.2 — Per-user reminder times + opt-out support:
  - Reads reminder_time and daily_reminder from UserProfile.preferences JSONB
  - Job runs hourly, only sends to users whose preferred time matches current ET hour
  - Users with daily_reminder=false are skipped entirely
  - Users with no profile or no preference default to 13:00 ET + opted in
  - HTML-escapes user names to prevent injection

Schedule this in your APScheduler setup:
    scheduler.add_job(send_daily_engagement_emails, 'cron', minute=0)
...
# ── Timezone anchor — matches platform convention ────────────────────────────
EASTERN = ZoneInfo("America/New_York")

# ── Defaults for users with no preferences set ──────────────────────────────
DEFAULT_REMINDER_TIME = "13:00"
DEFAULT_DAILY_REMINDER = True
...
DAILY_MESSAGES = [
    ...
    {"subject": "Your future self will thank you", "body": "Every check-in you complete is a data point. Every data point tells a story. That story, your story, is one of someone who chose to pay attention when it mattered.\n\nDon't break the chain. Check in today."},
    {"subject": "Money stress is real. So is your progress.", "body": "If money's been weighing on you, that's normal. Here's what's different now: you're measuring it. You're tracking it. You're not running from it.\n\nThat alone puts you ahead. Keep going."},
    ...
    {"subject": "Autopay your bills. Manual-pay your attention.", "body": "Automate the boring stuff. Never automate your awareness. The few minutes you spend checking in are the most valuable minutes in your financial day.\n\nYour check-in is waiting."},
    ...
    {"subject": "Your money habits are changing. Can you feel it?", "body": "Most people don't notice their own growth. Your FCS does. It's tracking every shift in how you think, feel, and act around money.\n\nCheck in today and see where you stand."},
    ...
    {"subject": "Emergency fund check: could you handle $500 today?", "body": "If a $500 surprise expense would stress you out, you're not alone. Knowing that, and tracking it, is exactly how you change it.\n\nYour check-in measures your emergency readiness. Let's see where you are today."},
    ...
    {"subject": "Don't just earn more. Understand more.", "body": "Income helps. Understanding your relationship with money? That's what GraceFinance measures. That's what actually changes your outcomes long-term.\n\nYour daily check-in is ready."},
...
# ══════════════════════════════════════════
#  HELPER: Get user's reminder preferences
# ══════════════════════════════════════════

def _get_user_reminder_prefs(profile: UserProfile | None) -> dict:
    """
    Extract reminder preferences from a UserProfile's JSONB.
    Returns defaults if profile is None or preferences are missing.
    """
    if profile is None:
        return {
    ...
    return {
        "daily_reminder": prefs.get("daily_reminder", DEFAULT_DAILY_REMINDER),
        "reminder_time": prefs.get("reminder_time", DEFAULT_REMINDER_TIME),
    }


# ══════════════════════════════════════════
#  SEND TO MATCHING USERS (runs hourly)
# ══════════════════════════════════════════

def send_daily_engagement_emails():
    """
    Send today's rotating motivational email to users whose preferred
    reminder time matches the current Eastern Time hour.

    Schedule with APScheduler to run at the top of every hour:
        scheduler.add_job(send_daily_engagement_emails, 'cron', minute=0)

    Flow:
      1. Determine current ET hour as "HH:00" string
      2. Load all verified users with their profiles
      3. For each user, read reminder preferences from JSONB
      4. Skip if daily_reminder is False (opted out)
      5. Skip if their reminder_time doesn't match current hour
      6. Send the email
    """
    now_et = datetime.now(EASTERN)
    current_hour_str = now_et.strftime("%H:00")

    logger.info(f"Email job running for time slot: {current_hour_str} ET")
    ...
    try:
        ...
        # Load all verified users with their profiles in one query
        users_with_profiles = (
        ...
        # Filter to users who should receive email this hour
        eligible_users = []
        for user, profile in users_with_profiles:
            prefs = _get_user_reminder_prefs(profile)

            # Skip users who opted out of daily reminders
            if not prefs["daily_reminder"]:
                continue

            # Skip users whose preferred time doesn't match this hour
            if prefs["reminder_time"] != current_hour_str:
                continue

            eligible_users.append(user)

        if not eligible_users:
            logger.info(f"No users scheduled for {current_hour_str} ET. Skipping.")
            return
        ...
        logger.info(
            f"Daily emails for {current_hour_str} ET: "
            f"{sent} sent, {failed} failed, {len(eligible_users)} eligible"
        )
