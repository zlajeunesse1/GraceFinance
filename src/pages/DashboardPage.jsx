/**
 * DashboardPage — Institutional Redesign
 *
 * Design: Monochrome. Data-forward. No decoration.
 * Removed: EcosystemLabels, MomentumMeter, ProgressionCard,
 *          GraceIndexPreview, DimensionRadar, StreakEngine,
 *          ModeToggle, Logo component, Badge, all emojis/glows/gradients,
 *          useTheme dependency (hardcoded monochrome tokens).
 * Kept:    FCS Score, QuickStats, TrendChart, DimensionBreakdown, GraceAI, NavBar.
 */
...
function getScoreMessage(score) {
  if (score >= 80) return "Strong position. Let's optimize."
  if (score >= 60) return "Solid foundation. More aware than most."
  if (score >= 40) return "Growing. Small consistent moves compound."
  if (score >= 20) return "Tough stretch, but you're showing up."
    ...
    <div style={{
      ...
      {items.map(function (item) {
        ...
        return (
          <button
            key={item.id}
            onClick={function () { props.navigate(item.path) }}
            style={{
              padding: "12px 20px",
              fontSize: 13,
    ...
    <Card style={{ textAlign: "center", padding: "36px 24px" }}>
      ...
      {trend !== null && trend !== undefined && (
        <div style={{ fontSize: 13, color: C.muted, fontFamily: FONT, marginBottom: 4 }}>
          ...
          <span style={{ marginLeft: 6 }}>vs last</span>
        </div>
      )}

      <p style={{
        fontSize: 13, color: C.dim, margin: "12px 0 0",
      ...
      {checkinCount > 0 && (
        <p style={{ fontSize: 11, color: C.faint, marginTop: 8, fontFamily: FONT }}>
          Based on {checkinCount} responses this week
        </p>
      )}
    </Card>
  )
  ...
  var stats = [
    {
      ...
      sub: score != null ? getScoreLabel(score) : "No data yet",
    },
    {
      label: "Streak",
      value: streak != null && streak > 0 ? streak + "d" : "0d",
      sub: streak != null && streak > 0 ? "consecutive" : "check in to start",
    },
    {
      label: "This Week",
      value: checkins != null ? String(checkins) : "0",
      sub: "check-ins",
    },
  ]
      ...
      {stats.map(function (s) {
        return (
          <Card key={s.label} style={{ padding: "20px" }}>
            ...
            <div style={{ fontSize: 12, color: C.dim, marginTop: 4, fontFamily: FONT }}>
              {s.sub}
            </div>
          </Card>
        )
  ...
  if (!hasData) {
    return (
      <Card>
        ...
        <p style={{
          fontSize: 13, color: C.dim, margin: "16px 0 0",
          lineHeight: 1.6, fontFamily: FONT,
        }}>
          Complete check-ins to reveal your dimension breakdown.
        </p>
      </Card>
    )
  }
            ...
            <div key={dim}>
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ color: C.text, fontSize: 13, fontWeight: 500, fontFamily: FONT }}>
                    {dimensionLabels[dim]}
                  </span>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  opacity: pct != null ? (pct > 60 ? 1 : pct > 30 ? 0.6 : 0.35) : 0,
                }} />
              </div>
            </div>
          )
...
function TrendChart(props) {
    ...
    return (
      <Card>
        ...
        <p style={{
          ...
          Check in for a few days to see your confidence trend over time.
        </p>
      </Card>
    )
  }

  var chartData = snapshots.map(function (s) {
  ...
  var avg = Math.round(
    chartData.reduce(function (a, b) { return a + b.fcs }, 0) / chartData.length * 10
  ) / 10
      ...
      <div style={{ height: 180 }}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
            ...
            <Area
              type="monotone" dataKey="fcs"
              stroke="#ffffff" strokeWidth={1.5}
              fill="url(#fcsGrad)"
              dot={{ r: 2.5, fill: "#ffffff", stroke: C.bg, strokeWidth: 2 }}
...
function GraceAICard(props) {
  ...
  if (score != null && score >= 70) {
    insight = "Score of " + score.toFixed(1) + " — strong. Let's optimize."
  } else if (score != null && score >= 40) {
    insight = "Score of " + score.toFixed(1) + " — real awareness. Let's explore what's driving it."
  } else if (score != null && score > 0) {
    insight = "Score of " + score.toFixed(1) + " — your baseline. Let's build from here."
  } else {
    insight = "Complete a check-in and I'll have personalized insights ready."
  }

  if (weakest && dimensionLabels[weakest]) {
    insight += " " + dimensionLabels[weakest] + " needs attention."
  }
  ...
  return (
    <Card>
      ...
      <p style={{
        ...
        margin: "12px 0 0", fontFamily: FONT,
      }}>
        Behavioral insights, not financial advice.
      </p>
    </Card>
  )
}
...
export default function DashboardPage() {
  ...
  var dateStr = todayDate.toLocaleDateString("en-US", {
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "24px 24px 60px" }}>
        ...
        {/* ── Date ── */}
        <p style={{
          fontSize: 12, color: C.dim, margin: "0 0 20px",
          letterSpacing: "0.02em",
        }}>
          {dateStr}
        </p>
